...
class simplesky {
  ...
  constructor(mapAPIID, mapCODE, darkskyKey, language = 'en', units = 'auto') {
    // this.mapAPIKey = mapsKey;
    this.mapAPIID = mapAPIID;
  ...
  async getWeather(location, lat, lng, exclude = [], extend = false) {
    var defaultQuery = await getQueryString(
      this.lang,
      this.units,
      exclude,
      extend
  ...
  async getTimeMachine(location, time, lat, lng, exclude = []) {
    let defaultQuery = await getQueryString(
      this.lang,
      this.units,
      exclude,
      false
  ...
  getCurrently(location, lat, lng) {
    ...
    return new Promise((resolve, reject) => {
      this.getWeather(location, lat, lng, excludeList)
        .then(response => {
  ...
  getHourly(location, lat, lng, extend = false) {
    ...
    return new Promise((resolve, reject) => {
      this.getWeather(location, lat, lng, excludeList, extend)
        .then(response => {
      ...
      this.getWeather(location, lat, lng, excludeList)
        .then(response => {
          if (response.minutely) {
            ...
            reject(
              new Error('Minutely data is not supported for this location')
            );
          }
        })
        .catch(error => {
      ...
      request(
        {
          url: `https://geocoder.api.here.com/6.2/geocode.json?app_id=${
            this.mapAPIID
          }&app_code=${this.mapCODE}&searchtext=${formattedLocation}`,
          json: true
        },
        (error, response, body) => {
          if (error) {
            reject(new Error('Unable to connect to Here Maps Services'));
          } else if (typeof body.Response == 'undefined') {
            reject(new Error('No address was passed in as a location.'));
          } else if (body.Response.View.length === 0) {
            reject(
              new Error(
                'Unable to find specified location, try reformating input'
              )
            );
          } else if (body.Response.View.length > 0) {
            let results = {
              lat:
                body.Response.View[0].Result[0].Location.DisplayPosition
                  .Latitude,
              lng:
                body.Response.View[0].Result[0].Location.DisplayPosition
                  .Longitude
  ...
  //         }, (error, response, body) => {
  //             if (error) {
  //                 reject(new Error("Unable to connect to Google Maps Services"));
